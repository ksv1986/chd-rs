...
impl Huffman {
    ...
    fn read_numbits_huffman(
        ...
        if code != numcodes {
            return Err(invalid_data(format!(
                "huffman:read_numbits_huffman: wrong number or huffman codes {}, expected {}",
                code, numcodes
            )));
        }
        Ok(())
    ...
    fn read_numbits_rle(&mut self, stream: &mut BitReader, nodes: &mut [Node]) -> io::Result<()> {
        ...
        let mut code = 0;
        while code < numcodes {
            let nodebits = stream.read(numbits) as ValueSize;
            if nodebits != 1 {
                // a non-one value is just raw
                nodes[code].numbits = nodebits;
                code += 1;
                continue;
            }
        ...
        if code != numcodes {
            return Err(invalid_data(format!(
                "huffman:read_numbits_rle: wrong number or huffman codes {}, expected {}",
                code, numcodes
            )));
        }
        if stream.overflow() {
            return Err(invalid_data_str(
                "huffman:read_numbits_rle: buffer too small",
            ));
        }
        Ok(())
    }
    ...
    fn assign_canonical_codes(&mut self, nodes: &mut [Node]) -> io::Result<()> {
        ...
        for (code, node) in nodes.iter().enumerate() {
            let numbits = node.numbits;
            if numbits > self.maxbits {
                return Err(invalid_data(format!(
                    "huffman:assign_canonical_codes: code {} bit number {} exceed maximal {}",
                    code, numbits, self.maxbits
                )));
            }
            if numbits <= 32 {
        ...
        for codelen in (1..32).rev() {
            ...
            if codelen != 1 && nextstart * 2 != (curstart + bithisto[codelen]) {
                return Err(invalid_data(format!(
                    "huffman:assign_canonical_codes: inconsistent starting codes {:x?}",
                    bithisto
                )));
            }
    ...
    fn build_lookup_table(&mut self, nodes: &[Node]) {
        ...
        for (code, node) in nodes.iter().enumerate() {
            ...
            // set up the entry
            let value = make_lookup(code, numbits);
